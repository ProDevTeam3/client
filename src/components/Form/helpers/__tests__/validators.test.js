import { dateOfBirthMatchesPESEL, NIPIsCorrect, PESELIsCorrect, requiredValue, sexMatchesPESEL, correctPostalCode, combineValidators } from "../validators"
...
describe("sexMatchesPESEL", () => {
  test("Returns error text when no sex passed", () => {
    const defaultText = "error";
    expect(sexMatchesPESEL(defaultText)(undefined, "49082134678")).toEqual(defaultText);
  });
  ...
  test("Returns error text if check fails", () => {
    const defaultText = "error";
    expect(sexMatchesPESEL(defaultText)("K", "49082134678")).toEqual(defaultText);
  })
});
...
describe("dateOfBirthMatchesPESEL", () => {
  test("Returns error text when no date passed", () => {
    const defaultText = "error";
    expect(dateOfBirthMatchesPESEL(defaultText)(undefined, "49082134678")).toEqual(defaultText);
  });
  test("Returns error text when no PESEL passed", () => {
    const defaultText = "error";
    expect(dateOfBirthMatchesPESEL(defaultText)("1949-08-21", undefined)).toEqual(defaultText);
  });
  test("Returns undefined if check passes", () => {
    expect(dateOfBirthMatchesPESEL("error")("1949-08-21", "49082134678")).toBe(undefined);
  });
  test("Returns error text if check fails", () => {
    const defaultText = "error";
    expect(dateOfBirthMatchesPESEL(defaultText)("2020-01-01", "49082134678")).toEqual(defaultText);
  })
});
...
describe("combineValidators", () => {
  test("Returns error text of first error", () => {
    const defaultText = "error";
    expect(combineValidators(
      requiredValue(defaultText)
    )(undefined)).toEqual(defaultText);
  });
